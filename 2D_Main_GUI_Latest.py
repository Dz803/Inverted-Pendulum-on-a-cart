...
# A scaling factor to convert control force magnitude to a length (tune as needed).
force_scale = 0.01  

# Create a FancyArrowPatch for the control force.
# Here, arrowstyle "->" produces a full arrow with a line and an arrowhead.
force_arrow = FancyArrowPatch((0, 0), (0, 0), arrowstyle="->", 
                               color="green", mutation_scale=20, linewidth=2)
ax_pendulum.add_patch(force_arrow)
...
def start_simulation(event):
    ...
    if sim_running and simulator.current_step < simulator.steps:
        ...
        ax_noise_plot.relim(); ax_noise_plot.autoscale_view()
        plt.pause(0.001)
    # After simulation, compute settling time.
    if filter_enabled:
        fs = 1 / dt_sim
        ...
        # Compute settling time on the raw theta_data.
        settle_time = compute_settling_time(np.abs(np.array(simulator.theta_data)), np.array(simulator.time_data), 0.01)
...
def update_pendulum(cart_x, theta):
    # Update the cart's rectangle position.
    ...
    pivot_x = cart_x
    pivot_y = cart_height  # Assume the pivot is at the top of the cart.
    ...
    # Retrieve the most recent control force; default to 0 if unavailable.
    if simulator is not None and len(simulator.F_data) > 0:
        u = simulator.F_data[-1]
    else:
        u = 0.0
    # Set the base of the arrow at the horizontal center of the cart and vertically at half cart height.
    arrow_base = (cart_x, cart_height/2)
    # The arrow tip is offset horizontally based on the force magnitude.
    arrow_tip = (cart_x + force_scale * u, cart_height/2)
    force_arrow.set_positions(arrow_base, arrow_tip)
